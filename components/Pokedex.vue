    ...
    <v-infinite-scroll height="100%" side="end" @load="loadMore" :loading="poke.loading">
      <v-row>
        <v-col v-for="pokemon in poke.pokemons.value" :key="pokemon.name" cols="12" sm="6" md="4" lg="3">
          <v-card :color="getPokemonColor(pokemon.detail?.types[0]?.type.name)" theme="dark" class="rounded-lg pa-4">
            <div class="d-flex">
              <div class="flex-grow-1">
                <v-card-title class="text-h5 pa-0 mb-2">{{ pokemon.name }}</v-card-title>
